...
async function sendCmd() {
  ...
  const voiceNote = args["--voice-note"]

  if (voiceNote) {
    if (args["--text"] || args["--file"]) {
      bail("--voice-note is exclusive with --text and --file")
    }
...
function statusCmd() {
  const bridge = createBridge()
  if (json) {
    console.log(JSON.stringify({ basic_features: true, advanced_features: bridge.available, typing_indicators: bridge.available, read_receipts: bridge.available, voice_note_send: bridge.available }))
  } else {
    console.log("imsg-plus Status Report")
    console.log("========================")
    console.log("\nBasic features (send, receive, history):\n  Available")
    console.log("\nAdvanced features (typing indicators, read receipts, voice notes):")
    if (bridge.available) {
      ...
      console.log("  imsg-plus send --to <phone> --voice-note <path.m4a>")
    } else {
